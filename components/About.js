...
const About = () => {
  return (
    <section className="bg-gray-800 text-white">
      <h2 className="text-center text-4xl font-bold pt-[5rem] pb-[3rem]">
        About{" "}
        <motion.span
        ...
        <div className="flex justify-center gap-[2rem] max-md:flex-col max-md:pr-10 max-md:pl-10">
          {SkillData.map((skill, index) => (
            ...
            >
              ...
              <div className="name mt-[0.5rem] mb-[0.5rem] font-bold text-[1.25rem]">
                {skill.name}
              </div>
              <div className="level">
                <progress value={skill.skill} className=""></progress>
              </div>
              <div className="mt-[0.5rem] flex items-center cursor-pointer transition-[.25s] hover:text-indigo-800">
                <FaExternalLinkAlt className="mr-[0.5rem]" />
                Details
              </div>
            </motion.div>
          ))}
